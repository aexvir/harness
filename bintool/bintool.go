...
type (
	// BinTool represents a single binary tool/versioncmd combination with the
	...
	// valid versioncmd of the command and installing the command in a way that is
...
// New initializes a BinTool with the provided command, versioncmd, and download
// url. Additional options may be provided to configure things such as the
// versioncmd test command, file extensions and folder containing the binary tool.
//
// The command, url, and versioncmd command may all use text templates to define
...
func New(command, version, url string, opts ...Option) (*BinTool, error) {
	return newcmd(command, version, url, opts...)
}

// NewGo initializes a BinTool with the provided go package and versioncmd.
// Additional options may be provided to configure things such as the versioncmd
...
func NewGo(pkg, version string, opts ...Option) (*BinTool, error) {
	...
	t, err := newcmd(cmd, version, pkg, opts...)
	if err != nil {
		return nil, err
	}
	t.gopkg = true
...
func newcmd(command, version, url string, opts ...Option) (*BinTool, error) {
	t := &BinTool{
		folder:     "./bin",
...
// IsInstalled checks whether the correct versioncmd of the tool is currently
// installed as defined by the versioncmd command.
func (t *BinTool) IsInstalled() bool {
	if t.tmplData.Version == "latest" || t.versioncmd == "" {
...
// Ensure checks to see if a valid versioncmd of the tool is installed, and
...
// WithVersionCmd defines a custom command used to test the versioncmd of the
...
// available to the url. If no command is provided, the versioncmd check will be
// skipped.
//
...
func (t *BinTool) installBinary() error {
	...
	if err := os.MkdirAll(t.folder, 0o755); err != nil {
		return fmt.Errorf("bintool: unable to create destination folder %s: %w", t.folder, err)
	}

	if err := os.WriteFile(t.tmplData.FullCmd, data, 0o755); err != nil {
...
func downloadAndExtract(command, url string) (data []byte, err error) {
...
func extractZipFile(r io.Reader, filename string) (data []byte, err error) {
	...
	for _, f := range zr.File {
		...
		defer fc.Close() //nolint:gocritic // copypasted code that needs refactor
