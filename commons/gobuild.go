...
// buildGoBuildArgs constructs the argument list for the go build command.
func buildGoBuildArgs(pkg, out string, conf buildconf) []string {
...
func GoBuild(pkg, out string, opts ...GoBuildOpt) harness.Task {
	...
	return func(ctx context.Context) error {
		args := buildGoBuildArgs(pkg, out, conf)
