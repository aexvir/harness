...
func TestGoBinaryOrigin(t *testing.T) {
	...
	t.Run("renames binary when package base name differs from template name",
		func(t *testing.T) {
			...
			tmpl := mktemplate(dir, "myimports", "latest")

			err := GoBinary("golang.org/x/tools/cmd/goimports").Install(tmpl)
			require.NoError(t, err)

			assert.FileExists(t, filepath.Join(dir, "myimports"))
			assert.NoFileExists(t, filepath.Join(dir, "goimports"))
		},
	)
	...
	t.Run("go install failure",
		func(t *testing.T) {
			dir := t.TempDir()
			tmpl := mktemplate(dir, "nonexistent", "latest")
