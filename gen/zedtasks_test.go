...
func TestUpsertZedTasks(t *testing.T) {
	t.Run(
		"creates file and appends tasks when missing",
		func(t *testing.T) {
	...
	t.Run(
		"re-run refreshes command/args; preserves user fields and unknown JSON keys",
		func(t *testing.T) {
...
func TestGenerateZedTasks(t *testing.T) {
	...
	t.Run(
		"generates entries for all mage targets",
		func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "tasks.json")
...
func TestCleanupZedTasks(t *testing.T) {
	...
	t.Run(
		"removes stale mage tasks; keeps active and non-mage; preserves comments",
		func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "tasks.json")
